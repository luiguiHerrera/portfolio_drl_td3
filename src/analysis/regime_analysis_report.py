...
def locate_and_load_histories(
    ...
    *,
    ...
    benchmark_dir = _benchmark_history_dir(metadata)
    for benchmark_name in DEFAULT_BENCHMARKS:
        ...
        if not path.exists():
            ...
            continue
        series = read_return_history(path)
...
def read_return_history(path: Path) -> pd.Series:
        ...
        rows.append(
            {
                ...
                "v3_rank_mandate_style": _rank_for(valid, "V3_cap_0.60", "rank_mandate_style_score"),
                "v4_rank_mandate_style": _rank_for(valid, "V4_cap_0.50", "rank_mandate_style_score"),
                "v3_minus_v4_mandate_style": (
                    _value_for(by_strategy, "V3_cap_0.60", "mandate_style_score")
                    - _value_for(by_strategy, "V4_cap_0.50", "mandate_style_score")
                ),
            }
        )
    return pd.DataFrame(rows)
...
def build_summary_markdown(
    ...
    v3_wins = _regime_ids_where_strategy_leads(rankings, "V3_cap_0.60")
    v4_wins = _regime_ids_where_strategy_leads(rankings, "V4_cap_0.50")
    ...
    v3_vs_gld = _pairwise_sentence(pair_available, "V3_cap_0.60", "BuyHold_GLD")
    v4_vs_gld = _pairwise_sentence(pair_available, "V4_cap_0.50", "BuyHold_GLD")
    v3_vs_trend = _pairwise_sentence(pair_available, "V3_cap_0.60", "trend_spy_cash_12p")
    v4_vs_trend = _pairwise_sentence(pair_available, "V4_cap_0.50", "trend_spy_cash_12p")
    return "\n".join(
        [
            ...
            "## Main Findings",
            "",
            f"- V3 leads by mandate-style score in: {_format_regime_list(v3_wins)}.",
            f"- V4 leads by mandate-style score in: {_format_regime_list(v4_wins)}.",
            (
                f"- Benchmarks lead by mandate-style score in {len(benchmark_wins)} "
                "regime/calendar slices."
            ),
            f"- V3 versus BuyHold_GLD: {v3_vs_gld}",
            f"- V4 versus BuyHold_GLD: {v4_vs_gld}",
            f"- V3 versus trend_spy_cash_12p: {v3_vs_trend}",
            f"- V4 versus trend_spy_cash_12p: {v4_vs_trend}",
            "",
            "## Interpretation",
            "",
            (
...
def _benchmark_history_dir(metadata: dict[str, Any]) -> Path:
    benchmark_dir = Path(metadata["benchmark_comparison_dir"]) / "benchmarks" / "histories"
