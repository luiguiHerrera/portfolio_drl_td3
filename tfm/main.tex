...
\textbf{Feasible action, cost, net return, and reward logic}

\begin{lstlisting}
Input:
    raw action a_t from the deterministic policy
    previous drifted portfolio weights w_prev_drifted
    next-period asset returns r_next
    asset-specific transaction cost rates kappa

w_t = projected version of a_t on the long-only simplex

turnover_i = abs(w_t[i] - w_prev_drifted[i]) for each asset i
transaction_cost_t = sum_i(kappa[i] * turnover_i)

gross_return_t = sum_i(w_t[i] * r_next[i])
net_return_t = gross_return_t - transaction_cost_t

portfolio_value_t = portfolio_value_prev * (1 + net_return_t)
running_peak_t = max(running_peak_prev, portfolio_value_t)
drawdown_t = portfolio_value_t / running_peak_t - 1

reward_t = net_return_t minus the configured drawdown penalty
store transition using feasible weights and net-return reward
\end{lstlisting}

\textbf{Walk-forward and benchmark-matched evaluation protocol}

\begin{lstlisting}
For each cash protocol in {Zero-CASH, BIL-CASH}:
    Build the same investable universe, cost assumptions, and date windows.

    For each walk-forward split:
        Train candidate TD3 policies only on the training window.
        Select candidates using the predefined diagnostic score stack.
        Evaluate selected candidates out-of-sample without exploration noise.

        Regenerate deterministic benchmarks on the same:
            asset universe
            cash protocol
    ...
    Aggregate out-of-sample results by candidate, cap, seed, and split.
    Interpret TD3 only against matched benchmark evidence.
\end{lstlisting}

\textbf{Statistical validation workflow}

\begin{lstlisting}
Build the searched TD3 candidate set from the final evaluation outputs.

For each selected TD3 candidate and clean benchmark comparator:
    Compute paired out-of-sample return differentials.
    Estimate the Sharpe-difference statistic.
    Bootstrap paired return histories to obtain:
        confidence interval for Sharpe difference
        probability that the selected TD3 candidate beats the comparator

Across the searched candidate set:
    Apply White Reality Check to candidate return differentials.
    Report the search-adjusted p-value.

Interpretation rule:
    ranking competitiveness may be reported when rankings support it;
    statistical superiority is not claimed when intervals include zero
    or White Reality Check does not reject the null.
\end{lstlisting}

\begin{flushleft}
The corresponding source areas are \path{src/env/portfolio_env.py}, \path{src/utils/action_projection.py}, \path{src/backtest/}, and \path{src/analysis/}. These references are sufficient to locate the reproducibility logic without exposing machine-specific paths.
\end{flushleft}
